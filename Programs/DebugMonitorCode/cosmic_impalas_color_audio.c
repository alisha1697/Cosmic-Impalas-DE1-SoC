...
//my defines
#define COLOR_OFFSET_REG (*(volatile unsigned short *)0x0040FFFE)
#define AUDIO_BASE ((volatile unsigned short*)0x0040FFF0)
...
void draw_sprite(byte* src, byte x, byte y) {
  ...
  for (row = 0; row < h; row++) {
      for (col = 0; col < w; col++) {
        dest = VIDEOMEM_ADDR(x + row, y + col); 
        *dest |= *src++;
      }
  }
}
...
byte xor_sprite(byte *src, byte x, byte y) {
  byte w, h, row, col, sprite_pixel, screen_pixel, result = 0;
  volatile byte *dest;

  w = *src++;          /* bytes in Y (vertical) direction  */
  h = *src++;          /* pixels in X (horizontal)         */
...
void move_player() {
  if(kbhit()) {
    char key = _getch();
    if(key == 'a' && player_x > 0) {
      erase_sprite(player_bitmap, player_x, 1);
      player_x -= 4;
      // draw_sprite(player_bitmap, player_x, 1);
    } else if(key == 'd' && player_x < 198) {
      erase_sprite(player_bitmap, player_x, 1);
      player_x += 4;
      // draw_sprite(player_bitmap, player_x, 1);
    } else if(key == ' ' && bullet_y == 0) { fire_bullet(); }
  }

  draw_sprite(player_bitmap, player_x, 1);
}

unsigned char bullet_sound[] = {
  0xFF, 0xE0, 0xC0, 0xA0, 0x80, 0x40, 0x00,
  0x10, 0x30, 0x50, 0x70, 0x90, 0xB0,
  0xD0, 0xF0, 0xD0, 0xB0, 0x90, 0x70,
  0x50, 0x30, 0x10, 0x00, 0x20, 0x00
};

unsigned char enemy_destroyed_sound[] = {
  0xFF, 0xEE, 0xDD, 0xCC, 0xBB, 0xAA, 0x99, 0x88,
  0x77, 0x66, 0x55, 0x44, 0x33, 0x22, 0x11, 0x00,
  0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88
};

// Player hit "popopopop"
unsigned char player_hit_sound[] = {
  0xFF, 0x00, 0xEE, 0x11,
  0xDD, 0x22, 0xCC, 0x33,
  0xBB, 0x44, 0xAA, 0x55,
  0x99, 0x66, 0x88, 0x77,
...
void play_array(unsigned char* sound, int len, int reps) {
  int i, j;
  for (j = 0; j < reps; j++) {
      for (i = 0; i < len; i++) {
          *AUDIO_BASE = sound[i];
...
void play_bullet_sound() {
  play_array(bullet_sound, sizeof(bullet_sound), 200); // longer pewwww
}

void play_enemy_destroyed_sound() {
  play_array(enemy_destroyed_sound, sizeof(enemy_destroyed_sound), 300); // fuller boom
}

void play_player_hit_sound() {
  play_array(player_hit_sound, sizeof(player_hit_sound), 500); // more popopopop
}
...
void update_next_enemy() {
  ...
  // COLOR_OFFSET_REG = (unsigned short)(offset);
  // offset = total_enemy_offset;
  // if (offset < 0) offset += 512;
  // else if (offset >= 512) offset -= 512;
  offset = (total_enemy_offset / 25) & 0x01FF;
  COLOR_OFFSET_REG = offset;
...
byte x, y;
{
    ...
    if (e) {
        delete_enemy(e);
        add_score(0x25);
        play_enemy_destroyed_sound(); //////////////////--------------------------------------------> added
    }
}
...
void cosmic_impalas_bonus_main() {
